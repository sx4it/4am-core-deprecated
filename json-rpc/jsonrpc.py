...
def getCallFunc(RPCRoot, methName):
	'''This function search beginning at RPCRoot, the methName'''
	current = RPCRoot
	for part in methName.split('.'):
		...
		if current is None:
			...
			break
	if (current is not None and (getattr(current, 'isJRPCCallable', None) is None
		or current.isJRPCCallable is not True)):
